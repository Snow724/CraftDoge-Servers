...
if ((speed player <= 1) && _canDo) then {
	if (s_player_settings < 0) then {
		s_player_settings = player addaction [("<t color=""#ffffff"">" + ("Set Viewdistance") +"</t>"),"CraftDodge\scripts\mission_settings\create.sqf","",5,false,true,"",""];
	};
} else {
	...
	*/
    ...
    if (_isMan and !_isAlive and !_isZombie and !_clothesTaken) then {
        if (s_player_clothes < 0) then {
            s_player_clothes = player addAction [("<t color='#0096ff'>")+("Take Clothes")+("</t>"), "CraftDodge\actions\player_takeClothes.sqf",[_cursorTarget], -10, false, true, "",""];
        };
    } else {
